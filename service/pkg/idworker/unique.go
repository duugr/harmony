...
func FormatTime(dateInt int64) string {
	...
	return objTime.Format(config.Configure.App.GetString("app.ymdhi"))
}
...
func FormatTimeByString(dateStr string) string {
	objTime, err := time.Parse(config.Configure.App.GetString("app.ymdhis"), dateStr)
	if err != nil {
		return ""
	}
...
func FormatStringToTime(dateStr string) int64 {
	...
	objTime, err := time.ParseInLocation(config.Configure.App.GetString("app.ymdhis"), dateStr, local)
	if err != nil {
		return 0
	}
...
func FormatDateStringToTime(dateStr string) int64 {
	local, _ := time.LoadLocation("Local")
	objTime, err := time.ParseInLocation(config.Configure.App.GetString("app.ymd"), dateStr, local)
	if err != nil {
		return 0
	}
