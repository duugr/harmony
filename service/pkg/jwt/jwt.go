...
import (
	...
	"github.com/duugr/harmony/service/pkg/idworker"
	"github.com/duugr/harmony/service/pkg/rsa"
)
...
func newJWT(exp int64, iss, aud string) *Algorithm {
	...
	return &Algorithm{
		...
		Jti: idworker.IdInt64(),
	}
}

func GenerateToken(data interface{}) string {
	exp := config.Configure.App.GetInt64("token.expired")
	iss := config.Configure.App.GetString("app.name")
	aud := config.Configure.App.GetString("app.sub.name")
...
func (a *Algorithm) encode() (token string, err error) {
	...
	byteToken, err := rsa.RsaEncrypt(orig)
	if err != nil {
